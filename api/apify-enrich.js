...
//
// Field mappings below have been validated against a real
// harvestapi/linkedin-profile-scraper response (Michael Howard profile, June 2026).
// experience[], certifications[], education[], skills[] are confirmed top-level
// keys. degree/field sub-fields on education entries remain best-effort, since the
// sample profile had no degree data to confirm against.
export default async function handler(req, res) {
  ...
  // Compute tenure in current role. The actor marks the ongoing role with
  // endDate: { text: 'Present' } (a truthy object, not an absent field), and
  // also provides a ready-made human-readable duration string per role
  // (e.g. "3 yrs 10 mos") which we prefer over computing from dates.
  function parseTenure(exp) {
    if (!Array.isArray(exp) || exp.length === 0) return null;

    const current = exp.find(e => e?.endDate?.text === 'Present'
      || (!e?.endDate && !e?.end && !e?.dateRange?.end)) || exp[0];

    if (current?.duration) {
      return `${current.duration} in current role`;
    }
    ...
    if (typeof startRaw === 'object' && startRaw.year) {
      const monthIndex = typeof startRaw.month === 'number'
        ? startRaw.month - 1
        : (MONTH_INDEX[startRaw.month] ?? 0);
      startDate = new Date(startRaw.year, monthIndex);
    } else {
